...
function parseEnv(content) {
  ...
  for (const line of content.split(/\r?\n/)) {
    ...
    // Strip surrounding quotes (dotenv-style). Keep # inside quoted values.
    if (
...
/** Quote values so dotenv does not treat # as a comment (passwords like David@123#). */
...
function mergeBotEnv(botDir, rootMap) {
  ...
  const lines = [];
  for (const [key, value] of botSpecific) lines.push(`${key}=${formatEnvValue(value)}`);
  for (const key of SHARED_KEYS) {
    if (merged.has(key)) lines.push(`${key}=${formatEnvValue(merged.get(key))}`);
  }
...
function main() {
  console.log("==================================================");
  console.log("  CASINOVA — SYNC workers/.env → all bot .env files");
  console.log("==================================================\n");
