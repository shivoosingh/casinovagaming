...
export async function loginToPanel(page: Page): Promise<void> {
  ...
  if (!onDashboard) {
    await page.goto(ADMIN_HOME, { waitUntil: "domcontentloaded", timeout: 45000 }).catch(() => {});
  ...
  if (username) {
    await page
      .locator(
        'input[name="username"], input[name="userName"], #username, .layadmin-user-login-username input'
      )
      .first()
      .fill(username)
      .catch(async () => {
        await page
          .locator('input:not([type="password"]):not([type="hidden"]):not([name*="code" i]):not([name*="vercode" i])')
          .first()
          .fill(username)
          .catch(() => {});
      });
  ...
  if (password) {
    await page
      .locator('input[name="password"], #password, .layadmin-user-login-password input, input[type="password"]')
      .first()
      .fill(password)
      .catch(() => {});
  }
