...
function pollMs(): number {
  const n = Number(env("CAPTCHA_POLL_MS") ?? 1000);
  return Number.isFinite(n) && n >= 500 ? n : 1000;
}
...
/** Read CAPTCHA from image. Prefer paid API when CAPTCHA_API_KEY is set (more reliable than OCR). */
export async function solveCaptchaImage(image: Buffer): Promise<{ text: string; method: "ocr" | "api" }> {
  const preferApi = (env("CAPTCHA_PREFER_API") ?? "true").toLowerCase() !== "false";
  const allowOcr = (env("CAPTCHA_OCR") ?? "false").toLowerCase() === "true";

  if (preferApi && env("CAPTCHA_API_KEY")) {
    const text = await solveWithPaidApi(image);
    return { text, method: "api" };
  }

  if (allowOcr && isAutoCaptchaEnabled()) {
  ...
  throw new Error("Could not read CAPTCHA — set CAPTCHA_API_KEY (OCR is off for speed)");
}
