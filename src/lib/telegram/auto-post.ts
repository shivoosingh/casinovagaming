...
const SITE_URL = process.env.NEXT_PUBLIC_SITE_URL || "https://Casinovacasinos.com";
...
export function formatBlogTelegramMessage(
  ...
  const header = options?.header || "🔥 <b>Casinova AI GAMING UPDATE</b> 🔥";
  const footer = options?.footer || "👉 Join Casinova & Claim Exclusive Bonus Credits! 🚀";
...
${keywordsTag} #Casinova #Gaming #OnlineCasino
`.trim();
}
...
export async function broadcastPromoToTelegram(
  ...
  const text = `
🎉 <b>SPECIAL Casinova PROMOTION</b> 🎉
...
#CasinovaPromos #CasinoBonus #FreeCredits
`.trim();
