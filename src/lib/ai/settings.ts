...
const DEFAULT_BLOG: BlogAiSettings = {
  ...
  target_keywords: ["casinova bonus", "juwa 777 download", "game vault tips"],
...
const DEFAULT_TELEGRAM: TelegramAiSettings = {
  ...
  template_header: "🔥 <b>CASINOVA GAMING UPDATE</b> 🔥",
  template_footer: "👉 Join Casinova & load your game desk today! 🚀",
...
const DEFAULT_CHATBOT: ChatbotAiSettings = {
  is_enabled: true,
  bot_name: "Casinova AI Assistant",
  system_prompt:
    "You are Casinova AI Assistant, a friendly gaming support bot. Help with game accounts (Juwa, Fire Kirin, Game Vault), deposits, cashouts, VIP tiers, and bonuses. Keep answers concise.",
...
const AI_MIGRATION_HINT =
  "Run supabase/final-file-features-casinova.sql in Supabase SQL Editor.";
...
export async function getBotSenderProfileId(): Promise<string | null> {
  if (process.env.CASINOVA_BOT_SENDER_ID?.trim()) {
    return process.env.CASINOVA_BOT_SENDER_ID.trim();
  }
