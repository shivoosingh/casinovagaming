...
const PREFETCH_ROUTES = ACCOUNT_LINKS.map((l) => l.href).filter(
...
export function HomeSidebar({
  ...
  function warm(href: string) {
    if (prefetched.current.has(href) || href === "/" || href.startsWith("/#")) return;
  ...
  useEffect(() => {
    import("@/lib/supabase/client").then(({ createClient }) => {
      ...
      void sb.auth.getUser().then(({ data: { user } }) => {
        ...
        if (loggedIn) {
          for (const href of PREFETCH_ROUTES) warm(href);
        }
      });
    });
  }, [router]);
  ...
  return (
    ...
    >
      <button
        ...
        className="group relative flex w-full items-center justify-center gap-2 overflow-hidden rounded-full py-3 text-sm font-bold text-gray-900 shadow-lg shadow-orange-500/25 transition-transform hover:scale-[1.02] active:scale-[0.98]"
      >
        <span className="absolute inset-0 bg-gradient-to-r from-orange-500 to-amber-500" />
        <span className="absolute inset-0 opacity-0 transition-opacity group-hover:opacity-100 bg-gradient-to-r from-amber-400 to-orange-500" />
      ...
      {!isLoggedIn ? (
        <>
          <div className="cx-glass-soft relative overflow-hidden rounded-2xl p-4">
            ...
            <Link
              href="/login"
              onMouseEnter={() => warm("/login")}
              className="relative z-10 block w-full rounded-xl border border-violet-400/35 bg-violet-500/20 py-2.5 text-center text-xs font-bold text-violet-100 transition-all hover:bg-violet-500/35"
            >
              Login & Access All
            </Link>
          </div>
          ...
          <div className="cx-glass-soft rounded-2xl p-4">
            ...
            <Link
              ...
              className="block w-full rounded-full bg-gradient-to-r from-orange-500 via-fuchsia-500 to-violet-600 py-2.5 text-center text-xs font-black text-white shadow-[0_8px_24px_rgba(232,121,249,0.35)]"
            >
              Sign Up
            </Link>
          </div>
        ...
        <div className="cx-glass-soft relative overflow-hidden rounded-2xl p-4">
          <div className="relative z-10 mb-2 flex items-center gap-2">
            ...
            <p className="text-sm font-bold text-white">Level Up Now</p>
          </div>
          <p className="relative z-10 mb-3 text-[11px] leading-relaxed text-slate-400">
            Unlock VIP rewards and exclusive perks.
          </p>
          <Link
            href="/dashboard/vip"
            onMouseEnter={() => warm("/dashboard/vip")}
            className="relative z-10 block w-full rounded-full bg-gradient-to-r from-orange-500 to-amber-500 py-2.5 text-center text-xs font-black text-gray-900"
          >
            View VIP Status
          </Link>
        </div>
      ...
      <div className="cx-glass-soft rounded-2xl p-4">
        ...
        <Link
          ...
          onMouseEnter={() => warm(isLoggedIn ? "/dashboard/messages" : "/support")}
