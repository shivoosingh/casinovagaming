...
interface CasinovaPosterPreset {
  id: string;
  name: string;
...
const Casinova_POSTER_PRESETS: CasinovaPosterPreset[] = [
...
export function AdminImageGeneratorCard() {
  ...
  const [preset, setPreset] = useState<CasinovaPosterPreset>(Casinova_POSTER_PRESETS[0]);
  const [offerText, setOfferText] = useState(Casinova_POSTER_PRESETS[0].offerAmount);
  const [dateText, setDateText] = useState("");
  const [domainText, setDomainText] = useState("Casinovacasinos.com");
  ...
  useEffect(() => {
    renderOfficialCasinovaPoster();
  }, [preset, offerText, dateText, domainText]);

  function renderOfficialCasinovaPoster() {
    ...
    // 2. LARGE BACKGROUND Casinova LOGO WATERMARK (Signature Brand Feature)
    ctx.save();
    ...
    // 4. Official Multi-Color Casinova Logo Emblem & Spiral at Top
    ctx.textAlign = "center";

    // Multi-color Casinova Spiral Icon
    ...
    ctx.fillText("Casinova", width / 2, 128);
    ...
    // White Pill Domain Name: Casinovacasinos.com
    ctx.fillStyle = "#ffffff";
    ctx.beginPath();
  ...
  function handleDownloadPoster() {
    ...
    link.download = `Casinova_brand_poster_${preset.id}_${Date.now()}.png`;
    link.href = dataUrl;
    link.click();
    toast.success("Downloaded Official Casinova Brand Poster with Logo Background (PNG)!");
  }
    ...
    <GlassCard className="p-6 border-amber-500/30 bg-background/80">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between border-b border-border/50 pb-4 mb-6 gap-2">
        <div>
          <h2 className="text-lg font-bold text-foreground flex items-center gap-2">
            ...
            Official Casinova Brand Poster Generator (With Logo Background)
          </h2>
          <p className="text-xs text-muted-foreground">
            Generate 100% official Casinova promotional poster images with the signature background Casinova logo watermark and real-time dates.
          </p>
        </div>
      ...
      <div className="mb-6 rounded-xl border border-amber-500/30 bg-amber-500/5 p-4">
        <p className="text-xs font-bold uppercase tracking-wider text-amber-400 mb-2.5 flex items-center gap-1.5">
          <Sparkles className="h-3.5 w-3.5" /> Select Official Casinova Poster Format
        </p>
        <div className="grid gap-2 sm:grid-cols-2 lg:grid-cols-5">
          {Casinova_POSTER_PRESETS.map((p) => (
            <button
              key={p.id}
              type="button"
              onClick={() => setPreset(p)}
      ...
      <div className="grid gap-6 lg:grid-cols-2">
        ...
        <div className="space-y-4">
          ...
          >
            <Download className="h-4 w-4" /> Download Official Casinova Poster (PNG)
          </Button>
        </div>
