...
export function AdminPlayerFollowupCard() {
  ...
  const [customSubject, setCustomSubject] = useState("👑 Welcome to Casinova Royale VIP — Claim Your $100 Bonus!");
  const [sending, setSending] = useState(false);

  function handleTemplateChange(type: "welcome" | "spin_reminder" | "weekend_bonus") {
    setTemplate(type);
    if (type === "welcome") {
      setCustomSubject("👑 Welcome to Casinova Royale VIP — Claim Your $100 Bonus!");
  ...
  function handleCopyWhatsAppLink() {
    const text = encodeURIComponent(
      "👑 *Casinova ROYALE VIP*: Your $100 Sign-Up Bonus Match is ready! Spin the daily bonus wheel free now: http://localhost:3001/dashboard"
    );
